x <- read.table("./gardasil.dat",header = T)
head(x)
...
reg <- glm(Completed~PracticeType,data=x, family = binomial)
summary(reg)
...
#Propensity score
library(mlogit)
xx <- mlogit.data(x,choice="PracticeType",shape="wide")
fit_bcl<-mlogit(PracticeType ~ 0 |Age +Race +InsuranceType+ MedAssist + LocationType,data=xx)
summary(fit_bcl)

x$PracticeType_bin <- as.numeric(as.numeric(x$PracticeType)>1) #2 -> 1, (0,1) -> 0
...
table(data$PracticeType_bin, subclass) #violates overlap


#creating subclasses
subclass.breaks = quantile(ps, c(.25, .50, .65)) # bins (initial try - modify as needed)
subclass = ps
subclass = as.numeric(ps>subclass.breaks[1])
subclass[which(ps>subclass.breaks[1]& ps<=subclass.breaks[2])]<- 1
subclass[which(ps>subclass.breaks[2]&ps<=subclass.breaks[3])]<- 2
subclass[which(ps>subclass.breaks[3])]<- 3
...
data$ps <-ps
check.overlap(x = data[which(ps>subclass.breaks[1]&ps<=subclass.breaks[2]),],
              trt = data$PracticeType_bin[which(ps>subclass.breaks[1]&ps<=subclass.breaks[2])],
              type = "both",
              propensity.func = prop.func)
...
data$ps <-ps
check.overlap(x = data[which(ps>subclass.breaks[2]&ps<=subclass.breaks[3]),],
              trt = data$PracticeType_bin[which(ps>subclass.breaks[2]&ps<=subclass.breaks[3])],
              type = "both",
              propensity.func = prop.func)
 ...
 data$ps <-ps
 check.overlap(x = data[which(ps>subclass.breaks[3]),],
               trt = data$PracticeType_bin[which(ps>subclass.breaks[3])],
               type = "both",
               propensity.func = prop.func)
...
tabUnmatched_s0 <- CreateTableOne(vars = vars, strata = "PracticeType_bin", data = data[which(subclass==0),], test = FALSE)
tabUnmatched_s1 <- CreateTableOne(vars = vars, strata = "PracticeType_bin", data = data[which(subclass==1),], test = FALSE)
tabUnmatched_s2 <- CreateTableOne(vars = vars, strata = "PracticeType_bin", data = data[which(subclass==2),], test = FALSE)
tabUnmatched_s3 <- CreateTableOne(vars = vars, strata = "PracticeType_bin", data = data[which(subclass==3),], test = FALSE)
